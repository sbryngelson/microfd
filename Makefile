# microfd: MPI always. `make` = NVIDIA, `make amd` = AMD, `make pvc` = Intel PVC, `make cpu` = host (no offload).
MPICC  ?= mpicc
ARCH   ?= cc80
...
PVCFLAGS = -O3 -qopenmp -fopenmp-targets=spir64 -DHOST_MPI
...
pvc: microfd.c
	I_MPI_CC=icx $(MPICC) $(PVCFLAGS) $(EXTRA) $< -o microfd -lm
cpu: microfd.c
...
.PHONY: nvidia amd pvc cpu test clean
