...
- MPI launch on this machine. UCX silently corrupts device buffers here, and `smcuda`'s CUDA IPC path corrupts every cross-GPU message after the first; `ob1` + `smcuda` with CUDA IPC disabled is correct. The tests default to this:

```bash
mpirun --mca pml ob1 --mca btl smcuda,self,vader --mca btl_smcuda_use_cuda_ipc 0 --mca coll_hcoll_enable 0 -np N ./microcfd key=value ...
```
