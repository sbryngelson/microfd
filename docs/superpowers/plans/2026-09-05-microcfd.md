...
- Only these OpenMP constructs: `target enter data`, `target update`, `target teams loop` (with `collapse` and `reduction`), `target data use_device_ptr`, `declare target` / `end declare target`. No `declare target` on globals; every kernel copies the scalars it needs into locals first.
...
- MPI launch on this machine. GPU peer copies are corrupt on this node, so CUDA IPC must be excluded; UCX with `^cuda_ipc` is correct and fastest. The tests default to this:

```bash
mpirun --mca coll_hcoll_enable 0 --mca pml ucx -x UCX_TLS=^cuda_ipc -np N ./microcfd key=value ...
```
...
**Python budget (added 2026-09-05).** The user requires the Python tooling to stay as compact as the solver. All tests live in one file, `tests/test.py` (one function per test, dispatched by name; shared `run` and `ok` helpers), and the whole of `tests/` stays under half the line count of `microcfd.c`. Tasks 4 and 5 therefore add functions to `tests/test.py`, not new scripts: `tgv2d` and `tgv3d` (Task 4, at most 12 lines each), `switches` and `sedov` (Task 5, at most 10 lines each), and `perf` (Task 5, at most 25 lines, invoked as `python3 test.py perf`). The code blocks below for those tasks predate this rule; the controller rewrites them to the `test.py` helpers when briefing each task.
...
MPIRUN = os.environ.get(
    "MPIRUN", "mpirun --mca coll_hcoll_enable 0 --mca pml ucx -x UCX_TLS=^cuda_ipc").split()
...
MPICC  ?= mpicc          # not CC: the environment here exports CC=nvc, which would bypass the wrapper
ARCH   ?= cc80
...
- `nvc` warns that `use_device_ptr` is deprecated in favour of `use_device_addr`. Keep `use_device_ptr` and accept the warning: on amdclang, `use_device_addr` applied to a pointer variable yields the device address of the pointer's own storage, not of the array, and the halo exchange then hands MPI host addresses (found on MI210, 2026-09-05).
...
Expected: all pass. Typical numbers: vortex order 2.0 with L1 density error about 2.1e-4 at 64 and 5.4e-5 at 128 (dimension-by-dimension FV WENO is formally second order in multi-D for nonlinear systems, Zhang, Zhang and Shu 2011; the error constant is ten times below MUSCL), octant difference below 1e-10, MPI fields bitwise equal. If MPI fields differ only in the last bits, it means some kernel reads a ghost value that was never exchanged; find it rather than switching to `allclose`.
...
- Test: `tgv2d` in `tests/test.py`
- Test: `tgv3d` in `tests/test.py`
...
Run: `cd tests && python3 test.py tgv2d`
...
Run:
```bash
make && cd tests && python3 test.py tgv2d && python3 test_mpi.py && python3 test_tgv3d.py
```
Expected: all pass. the `mpi` test is rerun because viscous tangential gradients read diagonal ghost cells, which is exactly what the sequential x-y-z exchange must provide. `test_tgv3d.py` takes several minutes. Typical 128^3 WENO5-Z result: peak dissipation 0.0118 to 0.0126 near t = 8.8 to 9.2.
...
**Files:**
- Test: `switches` in `tests/test.py`
- Test: `sedov` in `tests/test.py`
- Add: `perf` in `tests/test.py`
- Create: `README.md`
...
Run: `cd tests && python3 test.py switches sedov`
...
`make test` runs everything in `tests/`. `tgv3d` in `tests/test.py` takes a few
minutes. `python3 test.py perf` reports throughput.
...
|---|---|---|
| microcfd | FILL_IN | python3 test.py perf |
...
```bash
git add tests/test.py README.md
...
### Task 6: Performance pass (added 2026-09-06)

Approved by the user. Brief: `.superpowers/sdd/2026-09-05-microcfd/task-6-brief.md`. Experiments in order, each kept only if at least 2 percent faster at both 256^3 and 640^3 with the bitwise gate intact: periodic self-neighbour halo on the device without MPI; face-kernel trims (dead gradient components, thread_limit, maxregcount); pack/unpack launch reduction; one-kernel face for all directions if headroom remains; bandwidth-normalized README column. Baseline A100: 4.02 ns at 256^3, 4.21 at 640^3.

### Task 7: Python trim (added 2026-09-06)

Approved by the user. `tests/test.py` to at most half of `microcfd.c` (118 lines at 237), every check, tolerance and printed number preserved, exact Riemann solution bitwise identical. Brief: `.superpowers/sdd/2026-09-05-microcfd/task-7-brief.md`.
...
- Spec 4 MPI: Task 1 Cartesian setup and device selection by node-local rank; Task 2 exchange with `use_device_ptr` and `-DHOST_MPI`. Covered.
