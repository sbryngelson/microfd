...
def exact_rho(x, t, g=1.4, x0=0.5):              # exact Riemann density for Sod, ideal gas (Toro ch. 4)
    ...
    for _ in range(100):                         # Newton iteration for the star pressure
        (fl, dl), (fr, dr) = fk(p, rl, pl, cl), fk(p, rr, pr, cr); p = max(p - (fl + fr + ur - ul) / (dl + dr), 1e-12)
    ...
    for sg, (rk, uk, pk, ck) in ((-1, (rl, ul, pl, cl)), (1, (rr, ur, pr, cr))):   # left/right of the contact
        m = s <= u if sg < 0 else s > u; sm = s[m]; rs, cs = rk * (p / pk) ** (1 / g), ck * (p / pk) ** h
        rsh, S = rk * (p / pk + b) / (b * p / pk + 1), uk + sg * ck * np.sqrt((g + 1) / (2 * g) * p / pk + h)
        fan = rk * (2 / (g + 1) - sg * (g - 1) / ((g + 1) * ck) * (uk - sm)) ** e   # rarefaction fan
        rho[m] = np.where(sg * (sm - S) > 0, rk, rsh) if p > pk else np.where(     # shock ahead of / behind S
            sg * (sm - uk - sg * ck) > 0, rk, np.where(sg * (sm - u - sg * cs) < 0, rs, fan))
    return rho
...
def ic():
    ...
    ok(abs(d1[-1, 3] - 0.125) < 1e-12, "KE = 1/8"); ok(abs(d1[-1, 4] - 0.375 * f) < 1e-9, "enstrophy = 3/8 (sin dx / dx)^2")
...
def sod():
    ...
    for ax in range(3):
        ...
        rho[ax] = np.moveaxis(q[0], 2 - ax, 0)[:, 0, 0]; q0 = q if ax == 0 else q0   # line along the shock-tube axis
        ok(np.isfinite(q).all(), f"axis {ax}: finite fields"); ok(abs(d[-1, 1] - 0.2) < 1e-12, f"axis {ax}: reached t=0.2")
...
def vortex():   # dimension-by-dimension FV WENO5 is formally 2nd order in multi-D
    e = {}
    for n in (32, 64, 128):
    ...
    ok(p >= 1.9, "convergence order >= 1.9"); ok(e[32] > e[64] > e[128], "error decreases monotonically")
def wall():
...
def visc():   # 2D TGV Re=10: KE decays as exp(-4 nu t) to 1 percent; 3D TGV at Re=0.1 is Stokes flow: exp(-6 nu t) to 0.2 percent
    ...
    print(f"acoustic KE ratio = {r:.5f}, classical {ex:.5f}"); ok(abs(r / ex - 1) < 0.01, "acoustic absorption: bulk stress and conduction")
def tgv3d():   # 3D Taylor-Green Re=1600 at 128^3 vs the 512^3 spectral reference (minutes on one A100)
    ...
    ok(dev < 2.5e-3, "dissipation curve within 2.5e-3 of the reference (128^3 dissipates early)")
def sedov():   # 64^3 blast to t=0.1: finite, positive, and mass conserved (nothing reaches the outflow boundary yet)
    ...
    ok(abs(b[0].sum() / a[0].sum() - 1) < 1e-10, "mass conserved")
...
def perf():   # ns per cell per step on one GPU at 256^3, and weak scaling to 2 and 4 GPUs at 256^3 per GPU
    ...
    ok(t1 < 12, "must-level target: below 12 ns per cell per step")

for t in sys.argv[1:] or ["ic", "sod", "sedov", "vortex", "wall", "mpi", "visc", "tgv3d", "switches"]:
    globals()[t](); print("PASS", t)
