...
for ax in range(3):
    ...
    if ax == 0:
        q_axis0 = q                                          # full field, for the multi-rank check below
...
d2, r2 = run("sod_np2", 2, case="sod", axis=0, nx=200, ny=4, nz=4, px=2, nout=10**6, ndiag=10**6)
q2 = field(r2, last_step(r2), (200, 4, 4))
check(np.array_equal(q2, q_axis0), "1 rank == 2 ranks along x")
